...
public sealed class EnterTransition {
    ...
    override fun toString(): String = if (this == None) {
...
internal infix fun EnterTransition.withEffect(effect: TransitionEffect): EnterTransition = EnterTransitionImpl(TransitionData(effectsMap = mapOf(effect.key to effect)))

internal infix fun ExitTransition.withEffect(effect: TransitionEffect): ExitTransition = ExitTransitionImpl(TransitionData(effectsMap = mapOf(effect.key to effect)))
...
private fun Alignment.Horizontal.toAlignment() = when (this) {
    Alignment.Start -> Alignment.CenterStart
    Alignment.End -> Alignment.CenterEnd
    else -> Alignment.Center
}

private fun Alignment.Vertical.toAlignment() = when (this) {
...
internal operator fun <T : TransitionEffect> EnterTransition.get(key: TransitionEffectKey<T>): T? = data.effectsMap[key] as? T

@Suppress("UNCHECKED_CAST")
internal operator fun <T : TransitionEffect> ExitTransition.get(key: TransitionEffectKey<T>): T? = data.effectsMap[key] as? T
...
private class EnterExitTransitionModifierNode(
    ...
    var offsetAnimation: Transition<EnterExitState>.DeferredAnimation<IntOffset, AnimationVector2D>?,
    ...
    fun targetOffsetByState(targetState: EnterExitState, fullSize: IntSize): IntOffset = when {
...
private data class EnterExitTransitionElement(
    ...
    override fun create(): EnterExitTransitionModifierNode = EnterExitTransitionModifierNode(
        transition,
        sizeAnimation,
        offsetAnimation,
        slideAnimation,
