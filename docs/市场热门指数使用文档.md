...
var typeNameService = new TypeNameService();
var popularityService = new MarketPopularityService(db, typeNameService);
```
...
**计算公式**：
```
热门指数 = min(订单数/10, 25) + min(log10(供应量+1)*3, 25) + 近期订单占比*0.2 + 活跃小时数*5 + 时间聚类占比*0.15
```
...
1. **定期获取订单数据**
   ```csharp
   var marketService = new MarketDataService(esiApi);
   await marketService.GetTypeIdsFromOrdersAsync(regionId);
   ```

2. **运行分析**
   ```csharp
   var result = popularityService.Analyze(typeId, regionId);
   ```

3. **查看结果**
   - 热门指数越高，说明市场越活跃
   - 可以对比不同物品的热门指数来选择投资方向

---

## 注意事项

- 此分析基于**单次快照**的数据，建议定期获取订单数据积累更多信息
...
| 文件 | 说明 |
|------|------|
| `EVEMakeMoney/src/Data/MarketPopularityService.cs` | 热门指数分析服务核心逻辑 |
| `EVEMakeMoney/src/Data/MarketPopularityResult.cs` | 分析结果数据模型 |
| `EVEMakeMoney/src/Data/MarketOrderEntity.cs` | 订单实体类 |
| `EVEMakeMoney/src/Data/TypeNameService.cs` | 类型名称服务 |
